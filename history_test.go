...
func Test_history_size_limit_to_3(t *testing.T) {
	...
	if err := hk.Push("action1", 0, 1, nil, 3, false, expectFailed{}, "param1"); err != nil {
	...
	if err := hk.Push("action2", 1, 2, nil, 3, false, expectFailed{}, "param2"); err != nil {
		t.Fatalf("failed to push history item: %v", err)
	}
	...
	if err := hk.Push("action3", 2, 3, nil, 3, false, expectFailed{}, "param3"); err != nil {
		t.Fatalf("failed to push history item: %v", err)
	}
...
func Test_history_no_size_limit_stacktrace(t *testing.T) {
	...
	if err := hk.Push("action1", 0, 1, intentionalErr, 3, false, expectFailed{}, "param1"); err != nil {
		t.Fatalf("failed to push history item: %v", err)
	}
